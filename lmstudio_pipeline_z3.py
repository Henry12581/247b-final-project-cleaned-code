...
OUTPUT_DIR = "llm_weight_z3_code_output"
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "llama_3.2_3b_instruct_z3.txt")
...
def generate_response_worker(prompt, conn):
    try:
        response = client.chat.completions.create(
            model=MODEL_ID,
            messages=[
                {"role": "system", "content": "generate only z3 code in python"},
...
if __name__ == "__main__":
    with open(OUTPUT_FILE, "w", encoding="utf-8") as out_f:
        for i in range(21, 41):
            filename = os.path.join(PROBLEM_DIR, f"{i:02d}_weight_problem.txt")
