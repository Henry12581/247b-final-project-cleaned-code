...
stdout_log = open("deepseek_r1_distill_llama_8b_z3_cleaned_by_api.txt", "w", encoding="utf-8")
...
# ===== CONFIG =====
INPUT_FILE = "deepseek_r1_distill_llama_8b_z3.txt"
OUTPUT_FILE = "out.txt"
...
CLEAN_INSTRUCTION = (
    "You will be given a math problem and a response with multiple code blocks. "
...
def clean_problem_with_deepseek_stream(problem_text):
    try:
        ...
        stream = client.chat.completions.create(
            model=MODEL_ID,
            messages=[
                {"role": "system", "content": "Clean,debug and retain one working z3 python block only."},
    ...
    try:
        ...
        with open(OUTPUT_FILE, "w", encoding="utf-8") as out_f:
            for idx, prob in enumerate(problems):
                if not prob.strip():
                    continue
                print(f"\n=== Cleaning Problem {idx + 20} ===")  # Fixed: changed idx + 0 to idx + 1
